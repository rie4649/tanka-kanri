...
def fetch_bytes(url, tries=2):
    req = urllib.request.Request(
        url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"})
    last_err = None
    for i in range(tries):
        try:
...
def run_tokyotetsu():
    ...
    for href, dstr in links:
        ...
        url = urllib.parse.urljoin(TT_PAGE, href)  # ../assets/… のような相対リンクも正しいURLに直す
        pdfs.append((d, url))
    if not pdfs:
        print("東京製鐵: ページの先頭部分(デバッグ用) ----")
        print(html_text[:800])
        print("----")
    ...
    BUDGET_SEC = 8 * 60  # これを超えたら途中保存して、残りは次の実行で続きから
    skipped = 0
    for d, url in reversed(new_pdfs):  # 新しい日付から先に
        if time.monotonic() - start > BUDGET_SEC:
            skipped = len(new_pdfs) - (len(done_dates) - len(known_dates))
            print(f"  ⏱ 時間の都合でここまで。残り約{skipped}件は次回の実行で続きから取り込みます", flush=True)
            break
        try:
            data = parse_tokyotetsu_pdf(fetch_bytes(url), d)
        except Exception as e:  # noqa: BLE001
            print(f"  ⚠️ {d} のPDFを読めませんでした: {e}", flush=True)
            continue
        for ikey, (ad, price) in data.items():
            series.setdefault(ikey, {})[ad] = price
        done_dates.add(d)
        print(f"  📄 {d} 取り込みOK", flush=True)
        time.sleep(0.3)
